    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'turtle_controller.msg.RobotCmd')
            logger.debug(
...
class RobotCmd(metaclass=Metaclass_RobotCmd):
    ...
    def __init__(self, **kwargs):
        ...
        self.limit = kwargs.get('limit', float())
    ...
    def limit(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'limit' field must be of type 'float'"
            assert not (value < -3.402823466e+38 or value > 3.402823466e+38) or math.isinf(value), \
                "The 'limit' field must be a float in [-3.402823466e+38, 3.402823466e+38]"
        self._limit = value
